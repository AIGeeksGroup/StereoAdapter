...
class TartanairDataset(data.Dataset):
    ...
    def __getitem__(self, f_idx):
        ...
        if self.load_depth:
            ...
            depth_path_npy = depth_path.replace('.png', '.npy')
            inputs['depth'] = np.load(depth_path_npy)
        ...
        if self.dataset_mode == 'train':
            ...
            for key in list(inputs):
                if 'color' in key:
                    ...
                    # Compute GT disparity from GT depth
                    # disparity = disp_k / depth
                    disp_k = inputs['disp_k']
                    gt_disp = disp_k / depth  # avoid division by zero
                    mag = gt_disp.abs().squeeze(0)
                    valid = gt_disp > 0
                    inputs['valid'] = ((valid >= 0.5) & (mag < 192))
                    inputs['gt_disp'] = gt_disp.float()
        ...
        # resize for other modes
            ...
            for key in list(inputs):
                if 'color' in key:
                    ...
                    # Compute GT disparity from GT depth (test mode)
                    disp_k = inputs['disp_k']
                    gt_disp = disp_k / depth  # avoid division by zero
                    mag = gt_disp.abs().squeeze(0)
                    valid = gt_disp > 0
