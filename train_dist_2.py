...
parser.add_argument("--unfreeze_codyra",
                    dest="unfreeze_codyra",
                    type=bool,
                    default=False,
                    help="unfreeze CoDyRA layers when starting second param group",)
parser.add_argument("--is_codyra",
                    dest="is_codyra",
                    type=bool,
                    default=False,
                    help="check if using encoder with codyra",)
parser.add_argument("--step_epochs",
                    dest="step_epochs",
                    type=int,
                    nargs="+",
                    default=[20, 10, 20],
                    help="number of epochs for each step",)
...
class Trainer(object):
    def __init__(self, env_info):
        ...
        # Load the pretrained model or resume training
        if opts.resume is not None:
            self.network = self.saver.load_model(opts.resume, self.network)
            self.logger.print('# Resume from {}'.format(opts.resume))
        elif opts.pretrained_path is not None:
        ...
        if opts.unfreeze_codyra:
            self.network.net_module["encoder"].freeze_codyra()
        ...
        if opts.resume is not None:
            (self.optimizers,
             self.epoch,
             self.batch_step,
             extra_state) = self.saver.load_optim(opts.resume, self.optimizers)
            self.logger.print('# Resume optimizers from {}'.format(opts.resume))

            # 恢复 CoDyRA 训练状态
            if extra_state is not None:
                self.current_step = extra_state.get('current_step', 0)
                self.lora_frozen = extra_state.get('lora_frozen', False)
                self.logger.print(f'# Resume training state: current_step={self.current_step}, lora_frozen={self.lora_frozen}')

                # 恢复CoDyRA的dense/sparse状态
                if opts.is_codyra:
                    if 'codyra_dense_epoch' in extra_state:
                        # 从checkpoint恢复
                        dense_epoch = extra_state['codyra_dense_epoch']
                        current_sparse_epoch = extra_state['codyra_current_sparse_epoch']
                        sparse_epochs = extra_state['codyra_sparse_epochs']
                        self.logger.print(f'# Restoring CoDyRA state from checkpoint: dense_epoch={dense_epoch}, sparse_epoch={current_sparse_epoch}/{sparse_epochs}')
                    else:
                        # 旧checkpoint没有CoDyRA状态，根据当前epoch手动计算
                        self.logger.print('# Old checkpoint detected, calculating CoDyRA state from epoch...')

                        # 确定当前在哪个param_group
                        active_groups = [
                            (name, st_epoch)
                            for name, (_, _, st_epoch) in self.optimizers.items()
                            if self.epoch >= st_epoch
                        ]
                        current_group_idx = len(active_groups) - 1

                        if current_group_idx >= 0 and current_group_idx < len(opts.step_epochs):
                            # 获取当前group的总epoch数
                            total_epochs_in_group = opts.step_epochs[current_group_idx]

                            # 计算当前group的起始epoch
                            group_start_epoch = active_groups[-1][1]
                            epochs_passed_in_group = self.epoch - group_start_epoch + 1

                            # 获取dense_ratio (默认0.4，如果第一个block有这个属性就用它的)
                            dense_ratio = 0.4
                            if hasattr(self.network.net_module["encoder"], 'blocks'):
                                first_block = self.network.net_module["encoder"].blocks[0]
                                if hasattr(first_block.attn.qkv, 'dense_ratio'):
                                    dense_ratio = first_block.attn.qkv.dense_ratio

                            # 计算应该在哪个阶段
                            total_dense_epochs = int(total_epochs_in_group * dense_ratio)
                            total_sparse_epochs = total_epochs_in_group - total_dense_epochs - 1

                            if epochs_passed_in_group <= total_dense_epochs:
                                # 还在dense阶段
                                dense_epoch = total_dense_epochs - epochs_passed_in_group
                                current_sparse_epoch = 0
                            else:
                                # 已经在sparse阶段
                                dense_epoch = 0
                                current_sparse_epoch = epochs_passed_in_group - total_dense_epochs

                            sparse_epochs = total_sparse_epochs
                            self.logger.print(f'# Calculated CoDyRA state: group_idx={current_group_idx}, epoch_in_group={epochs_passed_in_group}/{total_epochs_in_group}')
                            self.logger.print(f'# CoDyRA state: dense_epoch={dense_epoch}, sparse_epoch={current_sparse_epoch}/{sparse_epochs}')

                    # 恢复所有CoDyRA layer的状态
                    if hasattr(self.network.net_module["encoder"], 'blocks'):
                        for block in self.network.net_module["encoder"].blocks:
                            if hasattr(block.attn.qkv, 'dense_epoch'):
                                block.attn.qkv.dense_epoch = dense_epoch
                                block.attn.qkv.current_sparse_epoch = current_sparse_epoch
                                block.attn.qkv.sparse_epochs = sparse_epochs
                            if hasattr(block.attn.proj, 'dense_epoch'):
                                block.attn.proj.dense_epoch = dense_epoch
                                block.attn.proj.current_sparse_epoch = current_sparse_epoch
                                block.attn.proj.sparse_epochs = sparse_epochs
                            if hasattr(block.mlp.fc1, 'dense_epoch'):
                                block.mlp.fc1.dense_epoch = dense_epoch
                                block.mlp.fc1.current_sparse_epoch = current_sparse_epoch
                                block.mlp.fc1.sparse_epochs = sparse_epochs
                            if hasattr(block.mlp.fc2, 'dense_epoch'):
                                block.mlp.fc2.dense_epoch = dense_epoch
                                block.mlp.fc2.current_sparse_epoch = current_sparse_epoch
                                block.mlp.fc2.sparse_epochs = sparse_epochs
                        self.logger.print('# CoDyRA state restored successfully')
        elif opts.start_epoch is not None:
            ...
            if opts.pretrained_path is not None:
                (self.optimizers,
                 self.epoch,
                 self.batch_step,
                 extra_state) = self.saver.load_optim(opts.pretrained_path,
                                                      self.optimizers)
    ...
    def _test_model(self):
        ...
        for inputs in self.val_loader:
            ...
            # 拆分成单样本来计算 metric（保证数值准确）
            for i in range(batch_size):
                # 提取第 i 个样本
                single_input = {}
                for key, value in inputs.items():
                    if isinstance(value, torch.Tensor):
                        single_input[key] = value[i:i+1]
                    else:
                        single_input[key] = value

                single_output = {}
                for key, value in outputs.items():
                    if isinstance(value, torch.Tensor):
                        single_output[key] = value[i:i+1]
                    else:
                        single_output[key] = value

                # 逐样本计算 metric
                for sou_name, metric_item in self.metric:
                    updated = metric_item.update_metric(single_output, single_input, name=sou_name)
                    if not updated:
                        nan_count += 1
                        break
        ...
        print(f"\n{nan_count} images skipped due to nan results")
        ...
        for sou_name, metric_item in self.metric:
            ...
            extra_state = {
                ...
                'lora_frozen': self.lora_frozen
            }

            # 保存CoDyRA的dense/sparse状态
            if opts.is_codyra and hasattr(self.network.net_module["encoder"], 'blocks'):
                # 获取第一个block的qkv layer的CoDyRA状态（所有layer共享相同的训练schedule）
                first_block = self.network.net_module["encoder"].blocks[0]
                if hasattr(first_block.attn.qkv, 'dense_epoch'):
                    extra_state['codyra_dense_epoch'] = first_block.attn.qkv.dense_epoch
                    extra_state['codyra_current_sparse_epoch'] = first_block.attn.qkv.current_sparse_epoch
                    extra_state['codyra_sparse_epochs'] = first_block.attn.qkv.sparse_epochs
                    self.logger.print(f'# Saving CoDyRA state: dense_epoch={extra_state["codyra_dense_epoch"]}, sparse_epoch={extra_state["codyra_current_sparse_epoch"]}/{extra_state["codyra_sparse_epochs"]}')
    ...
    def update_lora(self):
        if opts.is_codyra:
            ...
            if active_groups > self.current_step:
                self.current_step = active_groups
                print(f"Current step: {self.current_step}")

                if self.current_step > 0 and not self.lora_frozen:
                    self.network.net_module["encoder"].merge_weights()

                next_num_epochs = opts.step_epochs[active_groups - 1]
                self.network.net_module["encoder"].reset(next_num_epochs)
                if next_num_epochs <= 0:
                    self.lora_frozen = True
                else:
                    self.lora_frozen = False
        elif opts.freeze_lora and not self.lora_frozen:
            active_groups = [
                name
                for name, (_, _, st_epoch) in self.optimizers.items()
                if self.epoch >= st_epoch
            ...
            if len(active_groups) > 1:
                ...
                self.network.net_module["encoder"].freeze()
                self.lora_frozen = True
    ...
    def do_train(self):
        ...
        while self.epoch <= opts.epoch:
            ...
            if opts.is_codyra and not self.lora_frozen:
                self.network.net_module["encoder"].get_active_ranks()
                self.network.net_module["encoder"].next_epoch()
            ...
            if opts.save_freq is not None and self.epoch % opts.save_freq == 0:
                self.saver.save_model(self.network,
                                      ...
                                      extra_state={})
            # start testing
            if self.epoch % opts.test_freq == 0 or self.epoch == opts.epoch:
                self.logger.log_for_start_testing()
                with torch.no_grad():
                    self._test_model()
